...
def save_deposit_products(request):
    ...
    url = (
        f"http://finlife.fss.or.kr/finlifeapi/"
    ...
    for product in baseList:
        product_serializer = DepositProductsSerializer(
            data={
    ...
    for option in optionList:
        ...
        option_serializer = DepositOptionsSerializer(
            data={
...
def top_rate(request):
    ...
    if not best_option:
        return Response(
            {"error": "데이터가 없습니다."}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    return Response(
        {"product": product_serializer.data, "options": option_serializer.data}
    )
